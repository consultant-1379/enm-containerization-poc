...
cat<<-EOF
        ...
        Command Arguments
           ...
           -m, --mediation
               Includes corba NodePort & ingress service.
...
function process_arguments() {
  local short_args="mvfusdhx:p:n:a:w:r:l:c:"
  local long_args="mediation,privileged,force,recreate,sts,dep,help,extrasvc:,persist:,name:,svc:,wait:,requests:,limits:,cm:"
...
while true; do
  case "$1" in
     ...
     -v|--privileged)
        ...
        shift ;;
     -m|--mediation)
        export MEDIATION=true && echo MEDIATION=true >> ${TMP_FILE}
        shift ;;
     -h|--help)
        usage
        exit 0 ;;
...
if [ "$MEDIATION" != "true" ] ; then
  rm -f $NAME/templates/svc-corba.yaml
  rm -f $NAME/templates/svc-corba-ingress.yaml
  info "Removing mediation corba services"
fi
