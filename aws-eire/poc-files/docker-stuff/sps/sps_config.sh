...
cp /ericsson/tor/data/check_service_availability.sh /ericsson/3pp/jboss/bin/check_service_availability.sh
...
while [ $(curl -i -s http://localhost:12987  -w "%{http_code}" -o /dev/null) -ne 200 ]
do
  logger -t "SPS_RESTART" "SPS still not healthy, sleep 10...."
  sleep 10
done
